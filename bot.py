...
# === Сохранение / загрузка данных (не блокируем event loop) ===
async def save_data():
    try:
    ...
    for poll_id, data in list(active_polls.items()):
        try:
            ...
            try:
                ...
                # если успешно — считаем опрос восстановленным
                ...
                if "message to edit not found" in err_text or "message can't be edited" in err_text or "message is not modified" in err_text:
                    ...
                    # Если другая ошибка — всё же пытаемся сохранить привязку (на случай)
                    restored.append((poll_id, msg_id))
            except exceptions.TelegramAPIError:
...
# /commands - список команд (безопасный HTML: символы '<' экранированы)
...
async def cmd_help(message: types.Message):
    text = (
        ...
        " • /status — показать текущий опрос и кто что ответил\n"
...
async def main():
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        try:
            ...
            # Windows или окружение без сигналов
            pass
